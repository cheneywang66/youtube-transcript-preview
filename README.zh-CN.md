...
获取并总结带有时间戳的 YouTube 字幕，助你快速判断长视频是否值得观看。使用流程会先询问你的 `youtube-transcript.io` API token；如果你提供 token，抓取器会强制使用它的托管 REST API 拉取字幕。抓取结果仍依赖服务商、当前网络环境、YouTube 可访问性以及视频本身是否开放字幕。
...
2. 助手会先询问你的 `youtube-transcript.io` API token；如果你提供 token，它会使用 `https://www.youtube-transcript.io/api/transcripts` 拉取字幕。
3. 如果你在命令行直接使用，也可以提前设置 token：
   ```bash
...
助手将自动完成剩余工作：
- **询问 token**：先向你索取 `youtube-transcript.io` API token；拿到 token 后强制使用托管 API。
...
## 工作原理
1. **先询问 token**：触发 skill 后，助手会先询问 `youtube-transcript.io` API token。拿到 token 后，本次请求会强制使用托管 API。
2. **托管 API 拉取**：Python 脚本 (`scripts/fetch_transcript.py`) 会向 `POST /api/transcripts` 发送视频 ID，并把返回的 transcript tracks 规范化为带时间戳的 Markdown。
3. **免费方式获取字幕**：如果你明确表示没有 token 并同意继续，脚本仍可使用免费的 `youtube-transcript-api` 包和 YouTube 页面 `captionTracks` 获取字幕；优先中文，其次英文，最后尝试抓取任意可用的原始字幕。免费方式可能因为网络/IP 限制、区域访问或视频没有可用字幕而失败。
